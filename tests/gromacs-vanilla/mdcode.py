...
"""
        ...
        with open("stdout", "w") as stdout:
            with open("stderr", "w") as stderr:
                out = subprocess.run(
                    [executible], text=True, input=inp, stdout=stdout, stderr=stderr
                )
        ...
        with mda.coordinates.XTC.XTCFile(rundir + "/traj_comp.xtc") as xtc:
            for frame in xtc:
                if fnum == 0:
                    ...
                    pos = np.concatenate((pos, frame.x), axis=0)
                fnum = fnum + 1
        return pos
    ...
    def getCell(self, rundir):
        ...
        for frame in traj:
            ...
            if first:
                first, cell = False, np.array(
                    [[dim[0] / 10, 0, 0, 0, dim[1] / 10, 0, 0, 0, dim[2] / 10]]
                )
            else:
                box = np.array(
                    [[dim[0] / 10, 0, 0, 0, dim[1] / 10, 0, 0, 0, dim[2] / 10]]
    ...
    def getMasses(self, rundir):
        ...
        for i in range(nmols):
            masses[4 * k + 0] = 16.00000
            masses[4 * k + 1] = 1.00800
            masses[4 * k + 2] = 1.00800
            masses[4 * k + 3] = 0.00000
            k = k + 1
        return masses
    ...
    def getCharges(self, rundir):
        ...
        for i in range(nmols):
            charges[4 * k + 0] = 0.0
            charges[4 * k + 1] = 0.52422
            charges[4 * k + 2] = 0.52422
            charges[4 * k + 3] = -1.04844
            k = k + 1
        return charges
